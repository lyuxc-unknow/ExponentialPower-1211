...
public class GeneratorContainerMenu extends AbstractContainerMenu {
    ...
    @Override
    public @NotNull ItemStack quickMoveStack(@NotNull Player playerIn, int fromSlot) {
        ItemStack previous = ItemStack.EMPTY;
        Slot slot = this.slots.get(fromSlot);
        if (slot.hasItem()) {
            ItemStack current = slot.getItem();
            previous = current.copy();
            if (fromSlot == 0) {// From BE Inventory to Player Inventory
                if (current.getCount() <= 0) return ItemStack.EMPTY;
                if (!this.moveItemStackTo(current, 1, 36, true)) {
                    return ItemStack.EMPTY;
                }
            } else {// From Player Inventory to BE Inventory
